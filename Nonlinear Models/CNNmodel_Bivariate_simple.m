function CNNmodel_Bivariate_simple(channel1_index, channel2_index, data)
    ...
    YTrain = channel1Data(1, 1:train_size);  % Training target (next value of the first channel)

    XTest = [channel1Data(1, train_size + 1:end); channel2Data(1, train_size + 1:end)];  % Testing input from both channels
    YTest = channel1Data(1, train_size + 1:end);  % Testing target (next value of the first channel)

    % Reshape the data for CNN input (2 features per time step)
    XTrain = reshape(XTrain, [], 1, 2);   % [numTimeSteps, 1, numFeatures]
    YTrain = reshape(YTrain, [], 1);      % [numTimeSteps, 1]
    XTest = reshape(XTest, [], 1, 2);      % [numTimeSteps, 1, numFeatures]
    ...
    title(['CNN MODEL - Predicted vs Actual for Channels ' num2str(channel1_index) ' and ' num2str(channel2_index)]);
    ...
    % Optionally call the impulse response function
